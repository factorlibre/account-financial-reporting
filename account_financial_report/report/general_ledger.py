...
class GeneralLedgerReport(models.AbstractModel):
    ...
    def _get_period_ml_data(
        ...
        # Process the period move lines in memory-bounded batches (see
        # abstract_report._iter_move_line_data): a single search_read of every
        # line grew the ORM cache O(N) and exhausted the worker on full-year,
        # high-volume ledgers. Same result and order -- the ids are resolved
        # once with a single ordered search, not OFFSET pagination.
        for move_line in self._iter_move_line_data(domain, ml_fields):
